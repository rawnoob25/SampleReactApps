...
class App extends Component {
  ...
  /*
  *called before first render after construction
  */
  componentWillMount() {
    this.getDogs();
  }
  ...
  handleDeleteDog(name) {
    let dogs = this.state.dogs;
    let index = dogs.findIndex(x => x.name === name); //Array.prototype.findIndex() returns the first index that satisfies the testing function or -1 otherwise
    //(boolean-return function) passed as a parameter
    dogs.splice(index, 1); //Array.prototype.splice(x, y) removes y elements beginning at index x
    this.setState({dogs:dogs});
  }
